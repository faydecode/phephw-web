...
class C_admin_images extends CI_Controller {
	...
	public function index()
	{
		...
		*/
				...
				$config['first_url'] = site_url('toko-produk?'.http_build_query($_GET));
				$config['base_url'] = site_url('toko-produk/');
	...
	public function simpan()
	{
		...
		if (!empty($_POST['stat_edit']))
		{
			...
			$this->M_images->edit_with_image(
				...
				,$_POST['nama']
				,$foto
				,base_url().'assets/images/produk/'.$foto
				,$_POST['keterangan']
		...
		header('Location: '.base_url().'toko-produk-gambar/'.$id_produk.'?'.http_build_query($_GET));
	}
			...
			echo'<table width="100%" id="example2" class="table table-bordered table-hover">';
				...
				foreach($list_result as $row)
				{
					echo'<tr>';
						...
						if ($row->img_file == "")
						{
							$src = base_url().'assets/images/produk/loading.gif';
	...
	function do_upload($id,$cek_bfr)
	{
		...
		if($cek_bfr != '')
		{
			@unlink('./assets/images/produk/'.$cek_bfr);
		}
