...
class C_admin_history_penjualan extends CI_Controller 
{
	...
	{
		parent::__construct();
		$this->load->model(array('M_history_penjualan','M_public_get'));
	}
	...
	public function index()
	{
		...
		*/
				...
				else
				{
					...
					$tgl2 = date('d-m-Y'); 
					$tgl1 = $this->M_public_get->get_date($tgl2,7)->tgl;
				...
				$config['first_url'] = site_url('history-penjualan?'.http_build_query($_GET));
				$config['base_url'] = site_url('history-penjualan/');
				...
				$list_penjualan = $this->M_history_penjualan->list_penjualan(
					$tgl1,$tgl2,$faktur,$status,$this->uri->segment(2,0),$config['per_page']
				);
				
				
				$data=array('page_content'=>'admin_history_penjualan','halaman'=>$halaman,'tgl1'=>$tgl1,'tgl2'=>$tgl2,
	...
	function view_data()
	{
		...
		$list_penjualan = $this->M_history_penjualan->list_penjualan(
			$tgl1,$tgl2,$cari,$status,$config['per_page'],$this->uri->segment(2,0)
		);
