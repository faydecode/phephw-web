<?php
	class M_akun extends CI_Model 
	{
		...
		function get_no_costumer()
		{
			$query = $this->db->query(
			"
				SELECT CONCAT(FRMTGL,COALESCE(ORD,'0001')) AS no_costumer
				FROM
				(
					...
					(
						...
						MID(NOW(),9,2) AS D,
						(MAX(CAST(RIGHT(no_costumer,4) AS UNSIGNED)) + 1) AS ORD FROM tb_costumer
		...
		function get_no_karyawan()
		{
			$query = $this->db->query(
			"
				SELECT CONCAT(FRMTGL,COALESCE(ORD,'0001')) AS no_karyawan
				FROM
				(
					SELECT CONCAT(Y,M) AS FRMTGL
					 ,CASE
		...
		function get_no_faktur()
		{
			$query = $this->db->query(
			"
				SELECT CONCAT(FRMTGL,ORD) AS no_faktur
				FROM
				(
					SELECT CONCAT(Y,M) AS FRMTGL
					 ,CASE
					...
					(
						...
						CAST(MID(NOW(),6,2) AS CHAR) AS M,
						MID(NOW(),9,2) AS D,
						(MAX(CAST(RIGHT(no_faktur,4) AS UNSIGNED)) + 1) AS ORD FROM tb_h_penjualan
						WHERE kode_kantor = '".$this->session->userdata('ses_kode_kantor')."'
					) AS A
				) AS AA 
			"
        ...
        {
            $query = $this->db->get_where('tb_akun', array($berdasarkan => $cari));
            if($query->num_rows() > 0)
            {
		...
		function hapus($id)
		{
			$this->db->query('DELETE FROM tb_akun WHERE id_akun = '.$id.'');
		}
		...
		function get_profile($id_karyawan,$kode_kantor)
		{
			...
			$query = $this->db->query("
				SELECT A.id_karyawan,A.no_karyawan,A.nik_karyawan,A.nama_karyawan,A.pnd,
				A.tlp,A.email,A.avatar,A.alamat,B.nama_jabatan,C.id_akun,C.user,C.pass
				FROM tb_karyawan A
				LEFT JOIN tb_jabatan B
		...
		function cek_password($id_akun,$pass,$kode_kantor)
		{
			$query = $this->db->query("
				SELECT * FROM tb_akun
				WHERE id_akun = '".$id_akun."' AND kode_kantor = '".$kode_kantor."'
				AND pass = '".$pass."'
			");
		...
		function update_password($id_akun,$pass,$kode_kantor)
		{
			$query = $this->db->query("
				UPDATE tb_akun
				SET pass = '".$pass."'
